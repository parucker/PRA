...
int cria1GB (FILE* fp);
void escreveParcionado(FILE* fp, Apontador* arv);
int leArquivo (FILE* fr, Apontador *arv, int escolha,int index2);
...
void percorreArquivo(FILE* fr,int escolha, int inicio, int fim);
...
int main(int argc, char **argv)
{
	...
	while (i!=0){
		...
		if(i==1){
			...
			int quantidadeArq = leArquivo(fr,arv, escolha,index);
			...
			printf("Digite o inicio do intervalo e o fim");
			scanf("%d %d",&inicio,&fim);
			percorreArquivo(theOriginal,escolha,inicio,fim);
...
int cria1GB (FILE* fp){
	...
	static int index = 0;
	Corredor teste;
	for(i=0; i< GB/(sizeof(Corredor)); i++){
        index++;
...
int leArquivo (FILE* fr, Apontador* arv, int escolha,int index2){
	...
	while(!feof(fr)){
		...
		float tamanho = 131072000/(sizeof(Corredor));
		int anda = 0;
		//int parametro = 0;
		...
		arq = fopen(("%s",nome),"wb");
		escreveParcionado(arq, arv);
		printf("Saiu escreve parcionado\n");
		fclose(arq);
		//printf("Vai limpar a lista\n");
		//limpa(l); implementar limpa arvore
...
void escreveParcionado(FILE* fp, Apontador* arv)
{
  printf("Entrou escreve Parcionado\n");	
  em_ordem((*arv),fp);
}
/*
Lista* guardaArqMemoria(FILE* arq)
...
/*int ordenaCodigo(No* aux, No* atual)
{
	...
	//printf("Valor do codig,o novo: %d\n",aux->c.codigo);
	//printf("Retornou 0\n");
	return 0;
}
...
void percorreArquivo(FILE* fr,int escolha, int inicio, int fim)
{
  ...
  {
  	contador++;
  	fread(&aux,sizeof(Corredor),1,fr);
  	if(contador>=inicio && contador<=fim)
  	{
		...
		if(escolha == 3)
		{
			printf("Data %d:\t%d/%d/%d\n",contador,aux.data[0],aux.data[1],aux.data[2]);
		}
		if(escolha == 4)
		{
			printf("Tempo %d:\t%d:%d:%d:%d",contador,aux.tempo[0],aux.tempo[1],aux.tempo[2],aux.tempo[3]);
		}
	}
  }
}
